...
export default {
  ...
  computed: {
    ...
    filteredCategories() {
      ...
      if (this.categories?.video_banners) {
        filtered = this.categories?.video_banners.filter((category) =>
          category.title.toLowerCase().includes(this.searchQuery.toLowerCase())
        );
      }
  ...
  methods: {
    ...
    async deleteCategory(id) {
      if (confirm("Вы уверены, что хотите удалить этот элемент?")) {
        ...
        try {
          let link = '';
          if (this.propsPage.includes('1category')) {
            link = this.propsPage.slice(0, -9) + '-category';
          } else {
            link = this.propsPage;
            if (this.propsPage.includes('1banner')){
              link = this.propsPage.slice(0, -7) + '-banner';
            }
          }
          await axios.post(
            this.apiUrl + "api-" + link + "/del" + authGet,
            params,
            {
    ...
    getContent() {
      let cat = this.propsPage.includes('1category');
      let authGet = `&auth=${this.user.username}:${this.user.auth_key}`;
      if (this.propsPage === "theme" || this.propsPage === "object" || this.propsPage === "blogger" || this.propsPage === "video1banner" || cat) {
        let link = '';
        if (cat) {
          link = this.propsPage.slice(0, -9) + '-category';
        } else {
        ...
        axios
          .get(this.apiUrl + "api-" + link + "/get-list" + authGet)
          .then((response) => {
